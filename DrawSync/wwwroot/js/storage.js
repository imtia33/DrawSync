...
const Storage = {
    ...
    async deleteBoard(id) {
        await this.init();
        return new Promise((resolve, reject) => {
            const transaction = this.db.transaction([STORES.BOARDS, STORES.ELEMENTS], 'readwrite');
    ...
    async getElements(boardId) {
        ...
        return new Promise((resolve, reject) => {
            ...
            request.onsuccess = () => {
                const data = request.result ? request.result.strokes : [];
                // Migration logic: convert old strokes to pencil elements if needed
    ...
    async getStrokes(id) { return this.getElements(id); },
    async saveStrokes(id, data) { return this.saveElements(id, data); }
};
